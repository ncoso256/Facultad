...
public class NewMain {
    ...
    public static void main(String[] args) {
        ...
        while (diml < 20 && !visitante.equals("ZZZ")){
            ...
            partidos[diml] = new Partido(local,visitante,golesloc,golesvis);
            diml++;    
            visitante = Lector.leerString();
        }
        
        int i, cant = 0 , total= 0;
        for (i = 0; i < diml; i++){
            estilo = (partidos[i].getLocal() + "  " + partidos[i].getGolesLocal() + " VS " + partidos[i].getVisitante() + "  " + partidos[i].getGolesVisitante());
            System.out.println(estilo);
            if (partidos[i].getLocal().equals("River") && partidos[i].getGanador().equals("River") && !partidos[i].hayEmpate() || partidos[i].getVisitante().equals("River")&& partidos[i].getGanador().equals("River") && !partidos[i].hayEmpate())
                    cant++;
            if (partidos[i].getLocal().equals("Boca")&& !partidos[i].getVisitante().equals("Boca"))
                  total += partidos[i].getGolesLocal();  
        }
